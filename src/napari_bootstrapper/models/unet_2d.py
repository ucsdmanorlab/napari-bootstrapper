        ...
        for d in range(1, self.dims + 1):
            if x.size()[-d] % self.downsample_factor[-d] != 0:
                raise RuntimeError(
                    ...
                    f"mismatch in spatial dimension {self.dims - d}"
                )
...
class Upsample(torch.nn.Module):
    ...
    def __init__(
        ...
        if mode == "transposed_conv":
            ...
            up = {2: torch.nn.ConvTranspose2d, 3: torch.nn.ConvTranspose3d}[
                self.dims
            ]
    ...
    def crop_to_factor(self, x, factor, kernel_sizes):
        ...
        ns = (
            ...
            for s, c, f in zip(
                spatial_shape, convolution_crop, factor, strict=False
            )
        )
        target_spatial_shape = tuple(
            n * f + c
            for n, c, f in zip(ns, convolution_crop, factor, strict=False)
        )
        ...
        if target_spatial_shape != spatial_shape:
            ...
            assert all(
                (
                    (t > c)
                    for t, c in zip(
                        target_spatial_shape, convolution_crop, strict=False
                    )
                )
            ), (
                f"Feature map with shape {shape} is too small to ensure "
                f"translation equivariance with factor {factor} and following "
                f"convolutions {kernel_sizes}"
            )
    ...
    def crop(self, x, shape):
        ...
        offset = tuple(
            (a - b) // 2 for a, b in zip(x.size(), x_target_size, strict=False)
        )

        slices = tuple(
            slice(o, o + s)
            for o, s in zip(offset, x_target_size, strict=False)
        )
...
class UNet(torch.nn.Module):
    ...
    def __init__(
        ...
        for factor in downsample_factors[::-1]:
            if factor_product is None:
                ...
                factor_product = [
                    f * ff
                    for f, ff in zip(factor, factor_product, strict=False)
                ]
    ...
    def rec_forward(self, level, f_in):
        ...
        if level == 0:
            ...
            # convolve
            fs_out = [
                self.r_conv[h][i](fs_right[h]) for h in range(self.num_heads)
            ]
