...
def select_labels():
    @magicgui(
        call_button='Select labels',
        layout='vertical',
        apply3d=dict(widget_type='CheckBox', text='Apply in 3D', value=False, tooltip='Check box to keep label in 3D.')
    )
    def widget(
        viewer: napari.viewer.Viewer,
        labels_layer: napari.layers.Labels,
        ...
        if labels.ndim == 2 or (labels.ndim == 3 and apply3d):
            labels[np.isin(labels, label_ids, invert=True)] = 0
            ...
            for local_pt in local_points:
                ...
                labels2d[np.isin(labels2d, label_ids, invert=True)] = 0
